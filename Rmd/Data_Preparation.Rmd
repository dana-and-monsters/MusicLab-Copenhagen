...
```{r libraries}
packages = c("psych", "readxl", "magrittr", "reshape2","ggpubr", "ltm","janitor", "PerformanceAnalytics", "tidyverse", "lme4", "openxlsx")
...
```
## Add the userID col to the live participants spreadsheet
```{r}
setwd('../experimental-data/')
hall.ids <- read_excel("Concert_participants_installations.xlsx", sheet = "HallPartIDs")
```
```{r}
hall.ids<-rename(hall.ids, c("userID"="Installations"))
hallID<-hall.ids%>%select(userID, Code)

live.df$Code<-unlist(lapply(live.df$ID, function(x) {toupper(substring(x, 4))}))
test<-live.df%>%select(Code, ID)

live.df<-left_join(live.df, hallID, by = "Code")
test<-live.df%>%select(Code, userID)
```
...
### REMOVE WITHDRAWALS
There are actually more survey responses to remove. That one is the only for which we have both movement and survey data. 7 IDs with questionnaire results requested removal (their last request was for removal.) One is from before the copenhagen concert recording interval.
```{r}
...
df.full.rmwd<-df.full%>%filter(! userID %in% withdraw) # this removed 5 pts, 4 of which had incomplete datasets, one had a full dataset.

df.full<-df.full.rmwd
```
...
virt.dat<-df%>%filter(group == "Virtual")%>%
